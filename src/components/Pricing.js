  ...
  return (
    <section className='section-sm lg:section-lg bg-section'>
      <div className='container mx-auto'>
        ...
        <div className='flex flex-col mx-auto items-center gap-y-5 lg:flex-row lg:gap-x-[33px]'>{pricing.map((card, currentIndex) => {
          ...
          return (
            <div onClick={() => setIndex(currentIndex)}
              ...
              <div className='px-[30px] pt-[18px] pb-[30px] '>
                ...
                <ul className='bg-pink-100 flex flex-col gap-3 mb-10'>{list.map((item, index) => {
                  const { icon, name } = item;
                  return (<li className='border border-r-green p-4 rounded-md flex items-center gap-4' key={index}>
                ...
                <div className='absolute bottom-[30px] q-full left-0 right-0 px-[30px]'>
                  <button className={`${index === currentIndex ? 'bg-orange text-white' : 'border-orange'} w-full h-[50px] rounded border border-orange flex items-center justify-center text-orange
